...
""
 ...
 useEffect(() => {
    const cargarDatosFaltantesVehiculo = async () => {
      // Condición estricta: tenemos estadía e ID, pero no tenemos marca en el estado principal
      if (estadia?.id && patente && !estadia.marca) {
        try {
          const resVehiculo = await api.get(`/vehicles/buscar/?patente=${patente.toUpperCase()}`);
          if (resVehiculo.data?.sugerencias && resVehiculo.data.sugerencias.length > 0) {
            const vReal = resVehiculo.data.sugerencias[0];
            
            // Usamos un flag interno preventivo para evitar bucles de actualización
            setEstadia(prev => {
              if (prev && !prev.marca) {
                return {
                  ...prev,
                  tipo_vehiculo: vReal.tipo,
                  marca: vReal.marca,
                  modelo: vReal.modelo
                };
              }
          ...
          console.error("Error al rellenar datos del vehículo para el ticket:", error);
        }
      }
    };

    cargarDatosFaltantesVehiculo();
  }, [estadia?.id, patente]); // <-- Escuchamos solo cambios en el ID de la estadía y la patente string

  // 🖱️ 2. Selección manual limpia (Evitamos leer propiedades inexistentes de e.vehiculo)
  const seleccionarEstadia = (e) => {
    setEstadia(e);
    // Usamos encadenamiento opcional seguro: si e.vehiculo es un ID numérico, cae en e.patente
    setPatente(e.vehiculo?.patente || e.patente || "");
    setMonto(e.deuda);
    setSugerencias([]);
    setErrorMensaje("");
  };
  ...
  const crearPago = async (confirmado = false) => {
    ...
    try {
      ...
      const estadiaBase = res.data.estadia || estadia;

      const datosEstadia = {
      ...estadiaBase, // Conservamos todo el objeto original que ya funcionaba
      estado: "ABONADO",
      
      monto_abonado: parseFloat(monto),
      deuda: estadiaBase.deuda - parseFloat(monto),
      metodo_pago: metodo,
      fecha_pago: new Date()
      };

      //setEstadiaParaImprimir(datosTicket);

      onPagoExitoso(datosEstadia); // <-- Pasamos los datos del ticket al padre para imprimir
